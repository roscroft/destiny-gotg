...
def buildPvPTables(path, databasePath):
    def initializeTable():
        con = lite.connect(databasePath)
        with con:
            cur = con.cursor()
            cur.execute("DROP TABLE IF EXISTS PvPTotal")
            cur.execute("DROP TABLE IF EXISTS PvPPGA")
        ...
        for filename in os.listdir(path):
            if filename.endswith(".json"):
                with open(path+filename) as data_file:
                    ...
                    addToDatabase(totalStatTuple, 'PvPTotal')
                    addToDatabase(pgaStatTuple, 'PvPPGA')
...
if __name__ == "__main__":
    ...
    buildPvPTables(path, databasePath)
